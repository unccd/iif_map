...
Party = Backbone.Model.extend({
  decorateForDetailView: function(views) {
    ...
    return _.chain(views).map(function(view){
      ...
      var filterDef = app.filters.filterDefs.findWhere({name: filterAttribute});
      ...
      var option = app.filters.findWhere({attribute: filterAttribute, value: optionValue})
      if (option == undefined || optionTitle == '' ) { return }
